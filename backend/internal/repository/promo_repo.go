...
func (r *PromoRepo) GetByCode(code string) (*model.PromoCode, error) {
	p := &model.PromoCode{}
	var isActive, maxAct sql.NullInt64
	...
	err := r.db.QueryRow(
		...
		&expiresAt, &isActive, &p.CreatedAt)
	if err != nil {
		return nil, err
	}
	if maxAct.Valid {
...
func (r *PromoRepo) GetAll() ([]model.PromoCode, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.Code, &p.DiscountType, &p.DiscountValue, &maxAct, &p.ActivationsCount,
			&expiresAt, &isActive, &p.CreatedAt); err != nil {
			return nil, err
		}
		if maxAct.Valid {
			v := int(maxAct.Int64)
...
func (r *PromoRepo) Create(p *model.PromoCode) error {
	return r.db.QueryRow(
		...
		p.Code, p.DiscountType, p.DiscountValue, p.MaxActivations, p.ExpiresAt,
		boolToInt(p.IsActive), &p.ID,
	).Err()
}

func (r *PromoRepo) Deactivate(id int64) error {
	_, err := r.db.Exec(`UPDATE promo_codes SET is_active=0 WHERE id=:1`, id)
	return err
}
